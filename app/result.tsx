...
export default function ResultScreen() {
  ...
  const verdictText = isUncertain ? 'KHÔNG CHẮC CHẮN' : result.isFake ? t('result.fake') : t('result.real');
  const verdictIcon = isUncertain ? 'help-circle' : result.isFake ? 'alert-circle' : 'checkmark-circle';

  const fakePct = Math.round(result.fakeProbability * 100);
  const realPct = 100 - fakePct;

  // Với video: frames = timeline các khung (có anomaly = khung giả)
  const frames = result.frames || [];

  const shareReport = async () => {
    const report =
      `DeepGuard — Kết quả phân tích ${isVideo ? 'VIDEO' : 'ẢNH'}\n` +
      `Phán quyết: ${verdictText}\n` +
      (isVideo
        ? `Tỉ lệ khung nghi giả: ${result.frequency}\n`
        : `Xác suất Deepfake: ${fakePct}%\n`) +
      `Độ tin cậy: ${result.confidence}\n` +
      `Mô hình: ${model.name}\n` +
      `Thời gian: ${(result.latencyMs / 1000).toFixed(1)}s`;
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: c.bg }]}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        {/* Ảnh/video preview + badge */}
        <View style={styles.imageWrap}>
          <View style={[styles.imageBox, { borderColor: c.border }]}>
            {media ? <Image source={{ uri: media.uri }} style={styles.preview} /> : <Text style={{ color: c.textSecondary }}>—</Text>}
            {isVideo && (
        ...
        {/* Vòng tròn: video hiển thị % khung giả, ảnh hiển thị % deepfake */}
        <View style={styles.scoreWrap}>
          <View style={[styles.circle, { borderColor: verdictColor }]}>
            <Text style={[styles.score, { color: verdictColor }]}>
              {isVideo ? Math.round((result.blurScore || 0) * 100) : fakePct}
              <Text style={[styles.scoreSub, { color: verdictColor }]}>%</Text>
            </Text>
            <Text style={[styles.scoreCaption, { color: c.textSecondary }]}>
              {isVideo ? 'khung giả' : 'Deepfake'}
            </Text>
          </View>
          <Text style={[styles.scoreLabel, { color: c.textSecondary }]}>Độ tin cậy: {result.confidence}</Text>
        </View>

        {/* ===== TIMELINE KHUNG (chỉ video) ===== */}
        {isVideo && frames.length > 0 && (
          <View style={styles.timelineWrap}>
            <Text style={[styles.timelineTitle, { color: c.textPrimary }]}>
              Phân tích theo khung ({frames.length} khung)
            </Text>
            <Text style={[styles.timelineHint, { color: c.textSecondary }]}>
              Khung viền đỏ là nơi mô hình nghi ngờ giả mạo.
            </Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.timelineRow}>
              {frames.map((f, i) => {
                const fakePctFrame = Math.round((f.attention || 0) * 100);
                const isFakeFrame = f.anomaly;
                return (
                  <View key={i} style={styles.frameItem}>
                    <View style={[styles.frameThumb, { borderColor: isFakeFrame ? c.danger : c.success }]}>
                      <Image source={{ uri: f.uri }} style={styles.frameImg} />
                      <View style={[styles.frameTag, { backgroundColor: isFakeFrame ? c.danger : c.success }]}>
                        <Text style={styles.frameTagText}>{fakePctFrame}%</Text>
                      </View>
                    </View>
                    <Text style={[styles.frameLabel, { color: c.textMuted }]}>#{i + 1}</Text>
                  </View>
                );
              })}
            </ScrollView>
        ...
        {/* Thanh xác suất Thật/Giả (ảnh) hoặc tỉ lệ khung (video) */}
        <View style={styles.barWrap}>
          <View style={styles.barLabels}>
            <Text style={[styles.barLabel, { color: c.success }]}>
              {isVideo ? `Thật ${100 - Math.round((result.blurScore || 0) * 100)}%` : `Thật ${realPct}%`}
            </Text>
            <Text style={[styles.barLabel, { color: c.danger }]}>
              {isVideo ? `Giả ${Math.round((result.blurScore || 0) * 100)}%` : `Giả ${fakePct}%`}
            </Text>
          </View>
          <View style={[styles.bar, { backgroundColor: c.danger }]}>
            <View style={[styles.barFill, {
              backgroundColor: c.success,
              width: `${isVideo ? 100 - Math.round((result.blurScore || 0) * 100) : realPct}%`,
            }]} />
          </View>
        </View>
        ...
        <View style={[styles.techBox, { backgroundColor: c.accent }]}>
          ...
          <View style={styles.grid}>
            {isVideo ? (
              <>
                <Cell icon="film-outline" label="Khung phân tích" value={`${frames.length}`} c={c} />
                <Cell icon="warning-outline" label="Khung nghi giả" value={result.frequency} c={c} />
              </>
            ) : (
              <>
...
const styles = StyleSheet.create({
  ...
  videoBadge: { position: 'absolute', top: 10, right: 10, backgroundColor: 'rgba(0,0,0,0.6)', borderRadius: 14, padding: 6 },
  ...
  timelineWrap: { width: '100%', marginBottom: 22 },
  timelineTitle: { fontSize: 15, fontWeight: 'bold', marginBottom: 4 },
  timelineHint: { fontSize: 12, marginBottom: 12, lineHeight: 16 },
  timelineRow: { flexDirection: 'row' },
  frameItem: { alignItems: 'center', marginRight: 10 },
  frameThumb: { width: 70, height: 90, borderRadius: 8, borderWidth: 2, overflow: 'hidden' },
  frameImg: { width: '100%', height: '100%', resizeMode: 'cover' },
  frameTag: { position: 'absolute', bottom: 0, left: 0, right: 0, paddingVertical: 2, alignItems: 'center' },
  frameTagText: { color: '#fff', fontSize: 10, fontWeight: 'bold' },
